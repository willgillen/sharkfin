...
export const settingsAPI = {
  async getIconProviders(): Promise<IconProviderStatus> {
    const { data } = await apiClient.get<IconProviderStatus>("/api/v1/settings/icon-providers");
    return data;
  },

  async getLogoDevSettings(): Promise<LogoDevSettings> {
    const { data } = await apiClient.get<LogoDevSettings>("/api/v1/settings/logo-dev");
    return data;
  },

  async updateLogoDevSettings(settings: LogoDevSettingsUpdate): Promise<LogoDevSettings> {
    const { data } = await apiClient.put<LogoDevSettings>("/api/v1/settings/logo-dev", settings);
    return data;
  },
};
